...
COMMENT ON COLUMN mc_sgv_sac.cd_produto IS
    'Essa coluna recebe o Código do produto originário da tabela de Produtos.';

COMMENT ON COLUMN mc_sgv_sac.cpf_funcionario IS
    'Essa coluna recebe o CPF do funcionário responsável pelo atendimento';
...
COMMENT ON COLUMN mc_sgv_sac.dt_conclusao IS
    'Essa Coluna recebe a Data de Conclusão do Atendimento formalizada pelo funcionário em resposta ao Cliente.';

ALTER TABLE MC_SGV_SAC 
    ADD CONSTRAINT CK_CLASS_CHAMADO 
    CHECK (( ( dt_conclusao IS NOT NULL
                                                                   AND st_sac IN ( 'F', 'X', 'C' ) ) OR
...
ALTER TABLE mc_logradouro
    ADD CONSTRAINT fk_mc_bai_cd_bai FOREIGN KEY ( cd_bairro )
        REFERENCES mc_bairro ( cd_bairro );

ALTER TABLE mc_produto
    ADD CONSTRAINT fk_mc_catpro_cd_cat FOREIGN KEY ( cd_categoria )
        REFERENCES mc_categoria_prod ( cd_categoria );

ALTER TABLE mc_bairro
    ADD CONSTRAINT fk_mc_cidade_cd_cidade FOREIGN KEY ( cd_cidade )
        REFERENCES mc_cidade ( cd_cidade );

ALTER TABLE mc_visualizacao
    ADD CONSTRAINT fk_mc_cli_cd_cli FOREIGN KEY ( nr_cliente )
        REFERENCES mc_cliente ( cd_cliente );

ALTER TABLE mc_cli_juridica
    ADD CONSTRAINT fk_mc_cliente_cd_cliente FOREIGN KEY ( cd_cliente )
        REFERENCES mc_cliente ( cd_cliente );

ALTER TABLE mc_cli_fisica
    ADD CONSTRAINT fk_mc_cliente_cd_clientev2 FOREIGN KEY ( cd_cliente )
        REFERENCES mc_cliente ( cd_cliente );

ALTER TABLE mc_end_cli
    ADD CONSTRAINT fk_mc_cliente_cd_clientev3 FOREIGN KEY ( cd_cliente )
        REFERENCES mc_cliente ( cd_cliente );
...
ALTER TABLE mc_cidade
    ADD CONSTRAINT fk_mc_estado_cd_estado FOREIGN KEY ( cd_estado )
        REFERENCES mc_estado ( cd_estado );

ALTER TABLE mc_sgv_sac
    ADD CONSTRAINT fk_mc_func_cd_func FOREIGN KEY ( cd_funcionario )
        REFERENCES mc_funcionario ( mc_funcionario_id );

ALTER TABLE mc_end_func
    ADD CONSTRAINT fk_mc_func_cd_funcv2 FOREIGN KEY ( cd_funcionario )
        REFERENCES mc_funcionario ( cd_funcionario );

ALTER TABLE mc_sgv_sac
    ADD CONSTRAINT fk_mc_func_cpf_func FOREIGN KEY ( cpf_funcionario )
        REFERENCES mc_funcionario ( cpf_funcionario );

ALTER TABLE mc_funcionario
    ADD CONSTRAINT fk_mc_func_superior FOREIGN KEY ( mc_funcionario_id )
        REFERENCES mc_funcionario ( mc_funcionario_id );

ALTER TABLE mc_end_func
    ADD CONSTRAINT fk_mc_log_cd_log FOREIGN KEY ( cd_lougradouro )
        REFERENCES mc_logradouro ( cd_lougradouro );

--  ERROR: FK name length exceeds maximum allowed length(30) 
ALTER TABLE mc_end_cli
    ADD CONSTRAINT fk_mc_logradouro_cd_logradourov2 FOREIGN KEY ( cd_lougradouro )
        REFERENCES mc_logradouro ( cd_lougradouro );

ALTER TABLE mc_sgv_sac
    ADD CONSTRAINT fk_mc_produto_cd_produto FOREIGN KEY ( cd_produto )
        REFERENCES mc_produto ( cd_produto );

ALTER TABLE mc_visualizacao
    ADD CONSTRAINT fk_mc_vd_cd_vd_prod FOREIGN KEY ( cd_video,
                                                     cd_produto )
        REFERENCES mc_video ( cd_video,
                              cd_produto );

ALTER TABLE mc_video
    ADD CONSTRAINT fk_mc_vid_cdvid_prod FOREIGN KEY ( cd_produto )
        REFERENCES mc_produto ( cd_produto );

ALTER TABLE mc_video
    ADD CONSTRAINT fk_mc_video_cl_cd_class FOREIGN KEY ( cd_class )
        REFERENCES mc_video_class ( cd_class );

--  ERROR: No Discriminator Value found for FK FK_MC_CLIENTE_CD_CLIENTE - constraint trigger for Arc Arc_2 cannot be generated 

--  ERROR: No Discriminator Value found for FK FK_MC_CLIENTE_CD_CLIENTEv2 - constraint trigger for Arc Arc_2 cannot be generated
